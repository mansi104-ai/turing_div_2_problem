def solve():
    ...
    # Process all possible masks
    for mask in range(1, 1 << n):
        if bin(mask).count('1') <= 1:
            continue
            
        # Check if current mask is connected
        ...
        if len(visited) == len(nodes):  # Connected
            # Try all possible ways to split mask into two connected submasks
            submask = mask
            while submask > 0:
                submask = (submask - 1) & mask
                other = mask ^ submask
                if submask == 0 or other == 0:
                    continue
                ...
                # Check if the two submasks are connected by an edge
                connected_by_edge = False
                for u in nodes1:
                    for v in adj[u]:
                        if v in nodes2:
                            connected_by_edge = True
                            break
